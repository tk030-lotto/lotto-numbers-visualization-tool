...
- **現在のステータス**: **Phase 5 完了（4段階総合品質監査完了） / Phase 6 準備中（統合検証・ビルド監査・受入テスト）**

---

## 2. これまでに完了したこと（Phase 0 〜 Phase 5）
...
## 3. 次期フェーズの実装計画（Phase 6: 統合検証・ビルド監査・受入テスト）

次期フェーズでは、実際のブラウザ実行環境での動作検証および静的SPAとしての受入テストを実施します。

| ステップ | 検証項目 / 実行コマンド | 判定基準 |
| :--- | :--- | :--- |
| **6.1 アプリケーション全体統合確認** | 5くじ種切り替え、集計回数変更、出目表スクロール、モーダル表示 | エラーなく全UIが連動すること |
| **6.2 タイムトラベル過去検証** | 基準回号を過去回に変更 | 全指標・出目表が過去時点基準で正しく再計算されること |
| **6.3 オフラインフォールバック検証** | ネットワーク遮断時の動作確認 | 内蔵プリロードデータ（30〜50回分）で完全動作すること |
| **6.4 静的SPAビルド・差分監査** | `npm run build`<br>MCP `extract_git_diff` | 相対パス（`./`）での正常読み込み、GitHub Pages適合性 |
...
2. **【最厳守】AIコンテキスト管理MCPツールの積極利用**: `check_context_integrity`, `extract_git_diff`, `generate_phase_summary` などの定期実行。
